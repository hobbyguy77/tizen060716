		...
		$.ajax({
			...
			error: getStorage(function(res){
				for(var field in res){
					...
					$("#rssContent").append('<div class="feed"><div class="image"><img src=' + url_img + ' width=' + width +'px /></div><div class="title">' + title + '</div><div class="description">' + description + '</div></div>');
				}
	...
	function xmlParser(xml) {
		...
		$(xml).find("item").each(
				function() {
					...
					arr[i] = {url_img:url_img, title:title, description:description};
					setData(arr[i]);
					i++;
				});
	}
